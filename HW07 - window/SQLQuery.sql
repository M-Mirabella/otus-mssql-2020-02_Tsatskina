...
CROSS APPLY (SELECT SUM(invl.Quantity*invl.UnitPrice) as InvSum
			 ...
			 INNER JOIN Sales.Invoices inv_1 on invl.InvoiceID = inv_1.InvoiceID
...
select inv.InvoiceID, Customers.CustomerName, inv.InvoiceDate, invl.Quantity*invl.UnitPrice as SaleAmount,
#InvSums.InvSum As CumulativeAmount
FROM Sales.Invoices inv
INNER JOIN Sales.Customers on Customers.CustomerID = inv.CustomerID
INNER JOIN Sales.InvoiceLines invl on invl.InvoiceID = inv.InvoiceID
INNER JOIN #InvSums on #InvSums.InvoiceDate = inv.InvoiceDate
...
INNER JOIN Sales.Customers on Customers.CustomerID = inv.CustomerID
INNER JOIN Sales.InvoiceLines invl on invl.InvoiceID = inv.InvoiceID
INNER JOIN @InvSums as InvSums on InvSums.InvoiceDate = inv.InvoiceDate
ORDER BY inv.InvoiceDate
...
with cte_quantity as (SELECT sti.StockItemID, sti.StockItemName, MONTH(inv.InvoiceDate) as Month_InvDate,
SUM(invl.Quantity) as Sum_Quantity
FROM Sales.InvoiceLines invl 
INNER JOIN Sales.Invoices inv on inv.InvoiceID = invl.InvoiceID
INNER JOIN Warehouse.StockItems sti on sti.StockItemID = invl.StockItemID
...
SELECT sti.StockItemID, sti.StockItemName, sti.Brand, sti.UnitPrice, LEFT(sti.StockItemName, 1),
...
SELECT *
FROM (SELECT People.PersonID, People.FullName, Customers.CustomerID, Customers.CustomerName, inv.InvoiceDate,
(SELECT SUM(Invl.Quantity*Invl.UnitPrice) 
FROM Sales.InvoiceLines Invl WHERE Invl.InvoiceID = inv.InvoiceID) as SaleAmount,
ROW_NUMBER() OVER(PARTITION BY People.PersonID ORDER BY inv.InvoiceID DESC) AS rNumber
FROM Sales.Invoices inv
INNER JOIN Application.People on People.PersonID = inv.SalespersonPersonID
INNER JOIN Sales.Customers on Customers.CustomerID = inv.CustomerID) as tbl
WHERE rNumber = 1
ORDER BY PersonID

SELECT People.PersonID, People.FullName, Customers.CustomerID, Customers.CustomerName, Invoices.InvoiceDate,
(SELECT SUM(Invl.Quantity*Invl.UnitPrice) 
FROM Sales.InvoiceLines Invl WHERE Invl.InvoiceID = Invoices.InvoiceID) as SaleAmount
FROM (SELECT DISTINCT LAST_VALUE(inv.InvoiceId) OVER(PARTITION BY inv.SalespersonPersonID ORDER BY inv.InvoiceId ROWS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) as MaxID
FROM Sales.Invoices inv) as inv
INNER JOIN Sales.Invoices on Invoices.InvoiceID = inv.MaxID
...
SELECT *
FROM (SELECT DISTINCT Customers.CustomerID, Customers.CustomerName, invl.StockItemID, 
invl.UnitPrice, inv.InvoiceDate,
DENSE_RANK() OVER(PARTITION BY Customers.CustomerID ORDER BY invl.UnitPrice DESC) as rRank
FROM Sales.Invoices inv
INNER JOIN Sales.InvoiceLines invl on inv.InvoiceID = invl.InvoiceID
INNER JOIN Sales.Customers on Customers.CustomerID = inv.CustomerID) as tbl
WHERE rRank <= 2
