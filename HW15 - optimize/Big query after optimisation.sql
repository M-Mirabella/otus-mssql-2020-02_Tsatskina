...
CREATE TABLE @TmpTbl1 (
        ...
        select sum(f.[сумма с ндс]) as Сумма,
			...
			isnull(sv.КатегорияКод, kd.Код) as КатегорияКод, 
			isnull(sv.Знач2,-1) Категория,
			dog.[Бюджет-Договоры] идБюджета,
			isnull(sv.Примечание, kl.Код) as ОтрасльКод,
			isnull(sv.ОтрасльНазвание, kl.Название) as ОтрасльНазвание,
			org.Бюджет as БюджетОрг,
			isNull(dog.Окончание, '20450509') as Окончание                                 
        from stack.calc_contracts_accs(-10,@d1) f
        inner join stack.Договор dog on dog.ROW_ID = f.Договор
        inner join stack.Организации org on org.ROW_ID = dog.Плательщик
		OUTER APPLY (
					Select sv.Примечание, sv.Знач2, sv.Значение, kdp.Код as КатегорияКод, klp.Название as ОтрасльНазвание
					...
					Where sv.[Параметры-Договор] = dog.[row_id]					
					and @d1 between sv.ДатНач and sv.ДатКнц
					and sv.[Виды-Параметры] = 461) sv  --ДанныеПараметра("ВЫБОРОКВЭД")
...
group by f.АналитикаДок, f.Аналитика1, f.Аналитика2, f.Месяц, f.Договор, f.номномер, dog.Номер,
		...
		isnull(sv.КатегорияКод, kd.Код),
		isnull(sv.Примечание,kl.Код),isnull(sv.ОтрасльНазвание, kl.Название), org.Бюджет, isNull(dog.Окончание, '20450509')
